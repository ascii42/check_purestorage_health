...
#             guard; when set, raw API bytes are used for both alerts and perfdata
#             (for use once PureStorage firmware no longer returns transient counter spikes)
#   -ePatch/-eSW: show current array software version in verbose section headers
#   -ePatch/-eSW: fix jq crash when upgrade_hops is a string instead of array
#   -ePods: fix mediator status always showing "unknown"; read status from
#           .arrays[].mediator_status (per-array field) and address from .mediator
#
...
Options:
 ...
 --no-ni-spike-guard
    Disable the NI performance spike guard. By default, per-interface
    bytes/sec values that exceed the physical line rate are suppressed
    (zeroed in perfdata, no alert) because the PureStorage API can return
    transient inflated counters after a controller stats reset. Use this
    flag to pass raw API values through once the firmware issue is resolved.
 -u,  --bw-unit  <unit>
	...
	if [[ -n "${ni_perf_buffer}" && ! "${ni_perf_buffer}" =~ '"errors"' ]]; then
		...
		if [[ "${_niperf_total:-0}" -gt 0 ]]; then
			...
			for count in "${!_nip_name[@]}"; do
				...
				if [[ -z "${ni_spike_guard_disabled}" && "${_nip_max_b}" -gt 0 && "${_nip_bw_valid}" -eq 0 ]]; then
					_nip_perf_rx=0
					_nip_perf_tx=0
				fi
